...
/// With a plain bytecode-function reviver, pinned QuickJS enters 4,086 nested
/// `internalize_json_property` calls before its one-MiB stack budget rejects
/// the walk. This resumable state machine keeps those nodes on the heap, so the
/// logical budget is safe for the host stack while preserving that calibration
/// cutoff. Native and bound revivers shift the pinned cutoff; the fixed-budget
/// differences are recorded in `docs/deviations.md`.

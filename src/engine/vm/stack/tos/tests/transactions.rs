    ...
    for cached in [false, true] {
        for parameter in [false, true] {
            for mode in [StoreMode::Consume, StoreMode::Keep] {
                ...
                let destination = if parameter { window.parameters().start } else { window.locals().start };
                store.slots[destination] = Some(FrameBinding::Direct(JsValue::Object(object.into_handle())));
                let old;
                {
                ...
                assert!(matches!(store.slots[destination], Some(FrameBinding::Direct(JsValue::Int(42)))));
    ...
    for parameter in [false, true] {
        for mode in [StoreMode::Consume, StoreMode::Keep] {
            ...
            store.slots[destination] = Some(FrameBinding::Uninitialized);
            {
                ...
                assert!(cached_top(&slots));
                assert_eq!(slots.peek(0).unwrap(), &JsValue::Int(42));
                assert!(matches!(slots.store.slots[destination], Some(FrameBinding::Uninitialized)));
            }
            store.clear_frame(&runtime, window).unwrap();
        }
    }
    ...
    for cached in [false, true] {
        for pair in [false, true] {
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                if pair {
                    let _ = tx.slots().consume_number_pair(|_, _| panic!("number pair callback"));
                } else {
        ...
        let (mut store, mut window) = frame(&runtime, 2);
        {
            ...
            assert!(!tx.slots().binary_number(|_, _| panic!("declined callback")).unwrap());
            assert_eq!(tx.slots().consume_number_pair(|_, _| panic!("declined comparison")).unwrap(), None);
...
fn scalar_tos_number_local_callback_panic_preserves_local_and_cached_top() {
    for cached in [false, true] {
        ...
        assert!(matches!(store.local(&window, 0).unwrap(), FrameBinding::Direct(JsValue::Int(7))));
...
fn scalar_tos_number_callback_owning_output_is_committed_once_to_backing() {
    for cached in [false, true] {
        for source in ["({tag:42})", "'heap string result'", "170141183460469231731687303715884105727n", "Symbol('tos')"] {
            let runtime = Runtime::new();
            ...
            let mut calls = 0;
            {
                ...
                assert!(tx.slots().binary_number(|_, _| { calls += 1; output }).unwrap());
                let slots = tx.slots();
                assert!(!cached_top(&slots));
...
fn scalar_tos_number_local_capacity_and_compare_transactions_match_canonical() {
    for cached in [false, true] {
        ...
        let (mut store, mut window) = frame(&runtime, 2);
        {
            ...
            tx.slots().push(JsValue::Int(3)).unwrap();
            tx.slots().push(JsValue::Int(4)).unwrap();
            assert!(tx.slots().update_number_local(0, |previous| (previous.update(true), Some(previous))).is_err());
            assert!(matches!(tx.slots().local(0).unwrap(), FrameBinding::Direct(JsValue::Int(7))));
            assert_eq!(cached_top(&tx.slots()), cached);
            assert!(tx.slots().binary_number(|a, b| a.add(b).into()).unwrap());
            assert_eq!(tx.peek(0).unwrap(), &JsValue::Int(7));
            assert_eq!(cached_top(&tx.slots()), cached);
            assert!(tx.slots().update_number_local(0, |previous| (Number::Int(8), Some(previous))).unwrap());
            assert_eq!(tx.slots().consume_number_pair(|left, right| left.float() == right.float()).unwrap(), Some(true));
            assert_eq!(tx.slots().window.depth, 0);
            assert!(!cached_top(&tx.slots()));
            assert!(tx.slots().consume_number_pair(|_, _| panic!("underflow")).is_err());
        }
        assert!(matches!(store.local(&window, 0).unwrap(), FrameBinding::Direct(JsValue::Int(8))));
        #[cfg(feature = "profiling")]

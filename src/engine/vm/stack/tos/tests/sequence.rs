...
fn observe(slots: &RunSlots<'_>, completed: bool) -> Observation {
    let FrameBinding::Direct(local) = slots.local(0).unwrap() else { panic!("direct local"); };
    let FrameBinding::Direct(parameter) = slots.parameter(0).unwrap() else { panic!("direct parameter"); };
    #[cfg(feature = "profiling")]
    assert_eq!(slots.store.live_slots, 3 + slots.window.depth);
    Observation {
        completed,
        operands: (0..slots.window.depth).map(|offset| scalar_bits(slots.peek(offset).unwrap())).collect(),
    ...
    let mut state = 0x5eed_cafe_u32;
    {
        ...
        for step in 0..256 {
            ...
            let completed = match (state >> 16) % 12 {
                ...
                4 => slots.pop().is_ok(),
                5 => slots.binary_number(|left, right| left.add(right).into()).unwrap_or(false),
                6 => slots.consume_number_pair(|left, right| left.float() < right.float()).is_ok_and(|result| result.is_some()),
                7 | 8 => {
                    let mode = if state & 1 == 0 { StoreMode::Consume } else { StoreMode::Keep };
                ...
                9 => slots.update_number_local(0, |previous| (previous.update(true), Some(previous))).unwrap_or(false),

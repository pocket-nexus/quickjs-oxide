"""Path-sensitive ForIn domain guard and the helpers carrying its proof.
...
DEPENDENCIES = {
    'src/engine/object/internal_methods.rs': ('is_proxy_object', 'proxy_snapshot_if_any', 'internal_has_own_property', 'internal_snapshot_own_property_is_enumerable', 'internal_own_property_is_enumerable', 'internal_get_own_property'),
    'src/engine/object/properties.rs': ('get_own_property', 'get_own_property_in_operation', 'has_own_property', 'own_property_is_enumerable', 'own_property_keys', 'get_prototype_of'),
...
def check_local_arms(ctx, code, sources):
    for relative, names in DEPENDENCIES.items():
        ...
        for name in names:
            ...
            ctx.require_normalized_code_sha256('for-in-local-dependency',
                'non-Proxy ForIn helper must retain its reviewed no-JavaScript route: ' + relative + '::' + name,
    ...
    if start >= 0:
        return code[:start] + compact + code[end:]
    return code

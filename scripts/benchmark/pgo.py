...
def build(target, jobs, features, rustflags=None):
...
def main():
    ...
    parser.add_argument("--training-dir", type=Path, default=ROOT / "target/pgo-training")
    ...
    if not args.skip_training:
        for stale in profile_dir.glob("*.profraw"):
            stale.unlink()
    profile_dir.mkdir(parents=True, exist_ok=True)

    print("building instrumented CLI", flush=True)
    instrumented, _ = build(args.generate_target, args.jobs, [],
                            PROFILE_GENERATE.format(profile_dir))
    if not args.skip_training:

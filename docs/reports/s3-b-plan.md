...
**折叠实验（未采纳）**：把 S3 并入 `local_add_span` 成员、由 outlined
`numeric_local_add` 统一解析，并在 `supported` 路径加 `code[pc+2]==Add` 形状
门（避免转换驱动误认 S3 形态）后，BigInt 固定行累计回落到 +3.25%/+2.91%/
+1.84%、`prop_read` 707/轮，但 `int_local` 456.27 → 475.27（+4.2%）、
`empty_loop` 351.27 → 358.27（+2.0%）——热臂代码布局再次成为主导。结论：
该收益结构不值得换主指标回退，保留「独立 accessor + 末位分发块」形态。

**布局整理结论（本轮已做，未采纳源码改动）**：对 BigInt 固定行累计
+4.1%/+3.6%/+2.3% 做了专门归因与三次结构实验。事实链：① 临时移除 S3
分发块后 bigint32 精确回到 B2.1b 的 4,271M（+2.8% 累计），说明 B2.1c 的
+1.2% 是分发块对 `run` 代码布局的影响，与 S3 执行无关；② `perf annotate -l`
显示 base→B2.1a 的增热点落在 `run` 的 `PutLocal`/`replace_local` 存储路径
（`run.rs:1678`→`1709`，13.3%→21% cycles），即 BigInt Add 结果的写回
路径被 codegen 扰动；③ 折叠 accessor、constant-right 跳过探针（flags
41/42）、S3 块末位分发三种变体各只回收 0.1~0.8pp，且每次都以 `int_local`
或 `empty_loop` 主指标同量级回退为代价——布局收益在负载间是零和的。
结论：累计 2% 线无法通过局部源码改动稳定收敛；建议固定行门禁按 §6.2

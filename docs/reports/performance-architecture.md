...
| **B** | quickening + 可变执行 IR（QuickJS 没有）；首批只读 QuickOp 已随 C 回退，2026-09-24 重立项为守卫式专用执行跨度（B2.1）+ 条件自适应槽（B2.2） | 无 | 每操作指令数对齐为目标，量级待 B2.1 裁决 | [实施计划](s3-b-plan.md)（待裁决）；附录 A.1–A.3 |
| **D** | 数据导向堆：shape 一致性/免分配、typed arena、槽瘦身/内联、Map 记录与键哈希；D5 validity cell 条件启动（2026-09-24 收口：D1–D4 落地，D5 降级 backlog） | 无 | 内存 4–12× → ~2–3×；对象/数组密集 +10–30% | [实施计划](s3-d-plan.md) |
...
A4 决策点（已完成：保留 16B，见
[A4/D/B 决策报告](s3-a4-d-b-decision.md)）→ D（已完成：D1–D4 落地，
D5 降级 backlog，见
[阶段 D 实施计划](s3-d-plan.md) §4 实施记录）→ B 重新立项（先 spike/
设计）**。B 须按负结果重新设计并独立立项，
...
5. **B：quickening**：把已验证的 borrowed fast path 模式经特化 opcode
   ...
   2026-09-24 重立项为 [S3-B 实施计划](s3-b-plan.md)（待裁决）：
   B2.1 守卫式专用执行跨度（复用已认证 `FusionPlan`，guard 通过后
   不可失败、无 `Result`/owner 移动；失败原地 canonical），B2.2
   自适应 per-PC 专用槽仅在 B2.1 证明收益后条件启动；native 编组链
   为 B2.3 定向子项。
6. **D：数据导向堆**（2026-09-24 重订，同日收口）：按
   ...
   §4.1 保留 16B，不再作为 D 的前置；B 与 D 不混合提交。结果：对 D1a
   累计 objects RSS −21.7%、maps RSS −59.0%，map-int/map-string/
   map_delete 指令 −8.0%/−11.2%/−8.2%，prop_write −26.8%；D4 slice
   门禁缺口（指令 6.5–9.0%、Map RSS −18.6%）与 native 编组链一并归 B
   重新立项。D5 因启动门禁未满足降级 backlog。Test262 全量复跑无行级
   变化；基线晋升留作独立提交。

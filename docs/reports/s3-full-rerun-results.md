...
| --- | ---: | ---: | --- |
| bigint32 | 0.79–0.84 | 0.96 | 9028→8840 |
| bigint64 | ~0.90 | 1.05 | 9128→9012 |
| bigint256 | **1.21** | **1.32** | 9140→8940 |

> 口径更正（2026-09-23）：`target/final-run/fixedwork` 同源 CSV 复核显示
> final 构建的 instructions/pre_a 为 0.96 / 1.05 / 1.32；原值
> 0.98 / 1.12 / 1.43 与 head 构建（0.99 / 1.10 / 1.41）同源，属记录口径
> 混用，已按 final 直测更正。墙钟列不变。

bigint256 的回退（墙钟 +21%，指令数 +32%）自阶段 A 起未关闭，仍是当前最大单项回退；T1 窄修后发布协议（lto）下仍为 +35.6% 墙钟 / +29.0% 指令（见 [s3-a-plan.md §8.14](s3-a-plan.md)）。

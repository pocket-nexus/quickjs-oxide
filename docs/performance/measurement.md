...
所有分母用同一选定工具链／flags 重建；绝不混用旧二进制。当前计划选择 Rust 1.94.1 做新性能系列，并以 1.88.0 执行最低版本／CI 验证；#42 的 1.88.0 性能数字与 #43 的 1.94.1 数字保留在各自系列，不直接相除。更换性能工具链时整套分母重建，另起 series id。
...
使用普通 release、fat LTO、CGU=1，无 PGO、无 profiling。同机器串行完成构建，再开始测量；不能把“同一 CPU 绑核”当作允许别处并发测试的理由。记录 CPU 型号／微码／内核、可用 perf events、governor／频率策略、亲和性、内存压力和是否存在其他会话。共享主机不能排除干扰时，cycles／wall 标“未裁决”。

新系列的外部 v8-v7 统一 pin 为 `ahaoboy/js-engine-benchmark@2034d98fc8c5f8044e186267593f5d5ea5232caf`（#43 所用 checkout 的完整身份）。每个生成 bundle 单独记录 SHA-256；pin 相同不代表旧的 `dist/` 一定由它生成，必须重新生成并保存生成器身份。QuickJS 对照仍用项目 pinned 2026-06-04 oracle，记录 C 编译器与 flags；跨引擎是诊断参考，不代替同源码 A/B。

在每个干净 worktree 独立构建；以下参数已存在于仓库 `build.py`：

```sh
# 从该受测版本仓库根目录运行；OUT 必须位于仓库之外或被忽略。
OUT=$(mktemp -d /tmp/oxide-perf.XXXXXX)
RUSTUP_TOOLCHAIN=1.94.1 python3 scripts/benchmark/build.py \
  --jobs 2 --plain-target "$OUT/plain" --profile-target "$OUT/profile"
...
每个执行块候选保留 Base、Entry-only、Full 三种构建或等价的可解释对照。Entry-only 保留新入口／候选发现但不执行专用提交，用来观察准入与 codegen 税；Full 真正删除 canonical 工作。它不是纯净的因果分解：编译器可能因永不命中而删代码或改变内联，因此必须核对三份反汇编。必要时增加 Full 二进制上的自然命中／自然未命中配对负载，不能凭三个总数就断言某一机制成本。

固定诊断使用完全一致的工作量。先测同二进制 A/A，再用交错 ABBA／BAAB 顺序测 A/B；至少两次独立串行测量轮，每轮至少五个独立进程样本。保留原始样本和顺序，不只存中位数。确定性 instructions 不一致也要调查，不把其波动与 wall 调度噪声混为一谈。

```sh
# 单次采集示例；CPU 必须选主机允许的核，不能假定总有 CPU 2。
CPU=2
...
退休指令只数真正退休的指令，不包含错误预测后丢弃的执行。`cache-references` 的具体事件依 CPU，不能翻译成全部内存访问或几百倍命中率差。cycles、绝对 branch-misses／操作、CPU 支持的前端／错误推测／后端指标分开报告；多路复用或不支持的计数器明确标注。不能单凭 miss rate 上升判断退化。
...
python3 scripts/benchmark/run.py --suite v8-v7 --source "$SOURCE" \
  ...
  --repeat 5 --timeout 3600 --output "$OUT/v8-isolated"
python3 scripts/benchmark/run.py --suite v8-v7 --source "$SOURCE" \
  --engine base="$BASE_BIN" --engine candidate="$NEW_BIN" \
  --case all --repeat 5 --timeout 3600 --output "$OUT/v8-combined"
```
...
普通 release 的 perf／反汇编负责成本；profiling 构建负责逻辑事件，二者不能混成正式得分。报告实际符号归组规则、self／inclusive 口径、内联和采样 skid 的限制。父子调用时间不可重复加总，auth cache hit 不可代替 callsite 单态率。
...
| 固定工作量 | 完整输出校验、操作数、instructions/op、cycles/op、wall、分支事件、样本／离散度 |
...
本节是新拟定政策，不把历史波动伪装成已取得的统计保证。

**正确性硬门：** 每项定向差分与完整一致性门禁都要通过；只允许记录并核对基线本来存在的失败，不允许由性能 PR 修改允许向量来放行新增失败。

**性能硬门：** 每片及阶段累计均比较相同固定矩阵。BigInt32/64/256、已特化数值／属性循环、数组读写、call0、字符串簇、类型／TDZ异常负载，任一确定性退休指令增长 >2% 必须阻断默认开启并归因／回退；不能称“每片 <2%”就忽略累计 >2%。时间类在 A/A 可分辨后出现 >2% 稳定退化也阻断。不能判定的结果标“待测”，不当通过。

...
> 当前工作（2026-09-26）：继续削减 Array dense 恢复的分配与验证、非 method 调用的重复 callee 读取，将前／后缀增减方向编码到发布计划，并细分 materialized 失败诊断。2,152 项库测试通过；仅测三项短探针共 12 个进程，小幅指令变化不作收益准入。见[本轮实现与收据](receipts/cost-follow-up-2026-09-26/README.md)。

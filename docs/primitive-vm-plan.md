...
状态：2026-09-14，已确定继续使用**栈虚拟机**。S01–S07 已验收；S07 完整测量显示固定 58 项耗时为 PR19 的 1.17–5.28 倍，67 个成本样本零旧分派、零桥接。S08–S10 尚未实施；[回退分析](reports/primitive-vm-s07-performance.md)与[两阶段优化方案](primitive-vm-commit-plan.md#4-优化与最终交付)已更新，具体证据以逐 commit 计划和迁移清单为准。
...
一个 PR 合并为 **S01–S10 共 10 个提交**，保留三个关口：S01–S03 建立编译/发布边界、帧/槽所有权与原语栈核心；S04–S07 完成调用、内部回调、挂起和全部入口；S08–S10 优化、完整验收、默认切换与旧路径退出。诊断、测试和文档并入对应提交，原定语义与结构范围保持。

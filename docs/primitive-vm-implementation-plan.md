...
状态：2026-09-13，S01–S06 阶段验收通过，完整执行迁移尚未完成。执行表示已确定为栈 VM；本轮使用线性栈 IR、轻量控制流分析与有限融合。目标见[架构计划](primitive-vm-plan.md)，实施顺序合并为[10 个 commit](primitive-vm-commit-plan.md)，完整能力见[迁移清单](primitive-vm-migration.md)。

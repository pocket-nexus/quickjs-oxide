...
状态：继续实施。此前把候选处置等同于完整实现的状态已撤回；以下补充任务逐项验收，独立评审仍待完成。基于 PR #17 的 `b11f2be`，见 [PR #18](https://github.com/pocket-stack/quickjs-oxide/pull/18) 和[执行契约说明](published-execution-contracts.md)。
...
5. `clone_at_depth` 的基线 release 机器码仍有两个范围分支；改用切片迭代器的 rev().nth(depth) 选择尾部元素，不再显式 checked_sub 后 get。它仍在同一栈操作提交中单独验证 O(1) 选取和 root 克隆语义，检查最终机器码确实只保留一个范围条件。

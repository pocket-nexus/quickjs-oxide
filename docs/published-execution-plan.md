...
状态：发布/建帧抽象与部分重复检查消除已实现；其余优化待继续实施，独立评审待 PR。基于 PR #17 的 `b11f2be`，见 [PR #18](https://github.com/pocket-stack/quickjs-oxide/pull/18) 和[执行契约说明](published-execution-contracts.md)。
...
| E04 绑定访问 | E03 | 一起处理 local、argument、VarRef 的共享机制与静态模式检查 | 普通/词法/捕获语义测试、错误反例通过；逐项记录减少的工作与 loop/call/closure A/B | 部分实现：普通与 checked 读取优化及共享槽读写已落地；checked 写入/初始化/CloseLocal 尚待逐 opcode 拆分静态与动态条件。`a4b7830`。 |
| E05 常量与静态名字 | E03 | 种类明确的常量访问；复用链接 Atom；避免重复分类 | 常量种类混用在边界拒绝；root 生命周期与异常一致；执行、发布、内存数据齐全 | 访问入口已收拢，复用既有 Atom 表；避免重复种类分类尚未实现，当前保留 enum match。`51065a5` / `a23ec52`。 |
...
| E07 静态控制流与栈 | E03、E04 | 评估目标转换、PC 递增与栈检查；只迁移证明完整的部分 | 分支、finally、异常、恢复、回溯及畸形字节码覆盖；无 panic 替代原错误；记录保留项 | 优化未实现：当前保留目标、PC 和栈检查。继续证明普通顺序执行/静态分支的条件，与动态恢复/展开分别处理。 |
| E08 dispatch | E04、E05、E07 | 统一指令分类拥有者，评估减少多层分类；不改变操作语义 | opcode 覆盖完整，原 PC 对应不变；机器码/指令数与综合负载支持选型 | 分类结构已整理为单个 match，仍委派原语义处理器；尚未消除处理器内部的重复分派，未引入预解码表。`7a7fab7`。 |
| E09 集成与交接 | E06、E08 | 清除临时兼容入口与双实现；更新职责文档；完整正确性与性能复测 | 第 5、6 节验收齐全，保留无收益/退化结果，每项候选有最终处置 | 当前抽象和部分优化已做本地正确性验证；其余优化实现后重新集成验收，独立评审待 PR。 |
